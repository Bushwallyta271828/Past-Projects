...
int main()
{
    ...
    for (int top = 2; top < N + 1; top++)
    {
        for (int bot = 2; bot < N + 1; bot++)
        {
            ...
            {
                max_walks[top][bot] = max(max_walks[top - 1][bot], max(max_walks[top - 1][bot - 1] + 1, max_walks[top][bot - 1]));
            }
            else
            {
                max_walks[top][bot] = max(max_walks[top - 1][bot], max_walks[top][bot - 1]);
            }
        }
    }
